...
4.[1. 两数之和](https://leetcode-cn.com/problems/two-sum/)（6次）
...
5.[15. 三数之和](https://leetcode-cn.com/problems/3sum/)（6次）
...
3.[141. 环形链表](https://leetcode-cn.com/problems/linked-list-cycle/)（2次）
...
1.[20. 有效的括号](https://leetcode-cn.com/problems/valid-parentheses/)（1次）
...
*              i和j不断的往左右两边扩散，或者i和j往中间推进最后相遇，以及i和j写两层嵌套的循环枚举所有可能的i和j的情况。像这些代码一定要写的滚瓜烂熟。
*      3.栈 O(n)：创建一个有序的栈，当前元素的前一个元素就是它的左边界，如果大于栈顶元素继续入栈，如果发现比他小的可以确定它的右边界了，这时就弹栈计算面积。最后清空栈，计算栈中剩余元素的面积。
*/
```
...
/*
* 思路：1.暴力 O(n*k)
*      2.deque O(n)：所有滑动窗口的题目就想到用队列处理就行了。
*/
```
