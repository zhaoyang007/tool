...
4.[239. 滑动窗口最大值](https://leetcode-cn.com/problems/sliding-window-maximum/)（4次）
...
```js
/*
* 思路：1.hash O(nklogk)：用排序后的东西当作 key 存到一个 map 里面，然后对 map 的值进行归类。  
...
function groupAnagrams(strs) {
  ...
  for (let str of strs) {
    ...
    let list = map.has(count.join(',')) ? map.get(count.join(',')) : [];
    list.push(str);
    map.set(count.join(','), list);
  }
