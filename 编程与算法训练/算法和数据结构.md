...
1.[102. 二叉树的层序遍历](https://leetcode-cn.com/problems/binary-tree-level-order-traversal/)（7次）2021.08.05
...
3.[515. 在每个树行中找最大值](https://leetcode-cn.com/problems/find-largest-value-in-each-tree-row/)（7次）2021.08.06
...
4.[200. 岛屿数量](https://leetcode-cn.com/problems/number-of-islands/)（7次）2021.08.09
...
3.[122. 买卖股票的最佳时机 II](https://leetcode-cn.com/problems/best-time-to-buy-and-sell-stock-ii/)（3次）
...
1.[69. x 的平方根](https://leetcode-cn.com/problems/sqrtx/)（4次）
...
2.[367. 有效的完全平方数](https://leetcode-cn.com/problems/valid-perfect-square/)（3次）
...
3.[33. 搜索旋转排序数组](https://leetcode-cn.com/problems/search-in-rotated-sorted-array/)（2次）
...
动态规划和递归或分治没有本质上的区别，关键看有无最优子结构。如果没有最优子结构说明所有的子问题你都需要计算一遍，把最后的结果合并到一起，就是分治。

共性：找到重复子问题。

差异：有最优子结构，中途可以淘汰次优解。

用递归的方式将复杂的问题分解成简单的子问题。

一般来说动态规划的问题是让你来求一个最优解，最大值，一个最少的方式，正是因为它有最优子结构存在，那么你在中间的每一步就不需要把所有的状态都保存下来，只需要存那个最优的状态，最后推导出全局的最优。
...
1.斐波那契数列 1.傻递归（自顶向下） 2.记忆化递归 3.动态规划 for 循环递推（自底向上）
