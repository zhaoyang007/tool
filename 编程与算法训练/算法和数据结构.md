...
7.[53. 最大子序和](https://leetcode-cn.com/problems/maximum-subarray/)（7次）2021.08.22
...
内层循环每次把要排序的 i 算到有序序列的最后一项，从后向前遍历将 i 插入到相应位置。
...
在数组中去一个标杆 pivot，将小于 pivot 的元素放到 pivot 左侧，大于 pivot 的放到右侧，这样就会达到左侧的所有元素都小于右侧的所有元素，然后依次对左侧和右侧的子数组进行快排，也就是递归调用。这样整个数组就会达到有序的状态了。
