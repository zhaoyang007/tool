...
3.[122. 买卖股票的最佳时机 II](https://leetcode-cn.com/problems/best-time-to-buy-and-sell-stock-ii/)（7次）2021.08.14
...
1.[69. x 的平方根](https://leetcode-cn.com/problems/sqrtx/)（7次）2021.08.14
...
2.[367. 有效的完全平方数](https://leetcode-cn.com/problems/valid-perfect-square/)（7次）2021.08.14
...
3.[33. 搜索旋转排序数组](https://leetcode-cn.com/problems/search-in-rotated-sorted-array/)（7次）2021.08.15
...
1.[剑指 Offer 10- I. 斐波那契数列](https://leetcode-cn.com/problems/fei-bo-na-qi-shu-lie-lcof/)（5次）
...
3.[63. 不同路径 II](https://leetcode-cn.com/problems/unique-paths-ii/)（3次）
...
6.[120. 三角形最小路径和](https://leetcode-cn.com/problems/triangle/)（2次）
...
function minimumTotal(triangle) {
  ...
  let a = [];
    ...
    if (a[i] == null) a[i] = [];
    if (a[i][j] != null) return a[i][j];
    let value = Math.min(recursion(i + 1, j), recursion(i + 1, j + 1)) + triangle[i][j];
    a[i][j] = value;
		return value;
  }
  return recursion(0, 0);
