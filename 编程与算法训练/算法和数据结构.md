...
1.[283. 移动零](https://leetcode-cn.com/problems/move-zeroes/)（7次）2021.06.07（8次）2021.06.15
...
2.[11. 盛最多水的容器](https://leetcode-cn.com/problems/container-with-most-water/)（7次）2021.06.07（8次）2021.06.15
...
3.[70. 爬楼梯](https://leetcode-cn.com/problems/climbing-stairs/)（7次）2021.06.08（8次）2021.06.16
...
4.[1. 两数之和](https://leetcode-cn.com/problems/two-sum/)（7次）2021.06.10（8次）2021.06.18
...
5.[15. 三数之和](https://leetcode-cn.com/problems/3sum/)（7次）2021.06.10（8次）2021.06.20
...
3.[141. 环形链表](https://leetcode-cn.com/problems/linked-list-cycle/)（7次）2021.06.16
...
1.[20. 有效的括号](https://leetcode-cn.com/problems/valid-parentheses/)（7次）2021.06.17
...
2.[155. 最小栈](https://leetcode-cn.com/problems/min-stack/)（7次）2021.06.18
...
3.[84. 柱状图中最大的矩形](https://leetcode-cn.com/problems/largest-rectangle-in-histogram/)（1次）
...
4.[239. 滑动窗口最大值](https://leetcode-cn.com/problems/sliding-window-maximum/)（5次）
...
1.[242. 有效的字母异位词](https://leetcode-cn.com/problems/valid-anagram/)（1次）
...
function isAnagram(s, t) {
  ...
  let mapData = new Map();
  for (let i of s) {
    let value = mapData.has(i) ? mapData.get(i) + 1 : 1;
    mapData.set(i, value);
  }
  for (let i of t) {
    if (mapData.has(i)) {
...
2.[49. 字母异位词分组](https://leetcode-cn.com/problems/group-anagrams/)（1次）

```js
/*
* 思路：1.hash O(n*klogk)：排序后归类。  
*.     2.自制hash计数 + hash O(n*k)：自制 hash 后的数组当作 key 存到 map，然后对 map 的值进行归类。
*/
// 1
function groupAnagrams(strs) {
  ...
  for (let str of strs) {
    let key = [...str].sort().join('');
   	let value = mapData.has(key) ? mapData.get(key) : [];
    value.push(str);
...
现实中用的最多的就是二叉树，儿子节点只有两个，左儿子，右儿子。
...
基本的无序的树想查找一个元素就必须要遍历，就是把所有的节点都走一遍，其他的数据结构就是循环一遍一直到没有，树的话会有左子树右子树，那就类似的递归去把它反复的求证。树的定义本身就决定了它没法有效的进行循环，而写递归调用相对比较简单。所以树的各种操作不要怕递归，要拥抱递归。
...
![1624102233925](../../Downloads/1624102233925.jpg)
...
* 以此类推，左右子树也都满足这两个条件为二叉搜索树。（这就是重复性）
