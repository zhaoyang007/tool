...
.vue 文件  vue-loader 编译成  => js css => 将编译好的 js 和 css 容纳于 html 中。

按照框架的规范去写。里面操作 dom 和，数据驱动视图的工作都是由 vue.js 来帮我们去完成。

所以我们现在要学习的就是 vue 中的规定的写法，来开发一个项目。

还有 vue 社区比较流行的组件库。elment-ui

我会在以后的学习中反复的提到我们的上面介绍的概念和思想，来帮助大家逐渐对 vue 的学习有一个更加全面和深刻的认识。

Vue 相比于传统开发的优点：

1. 不用手动操作 dom，采用数据驱动的方式，使开发开发效率大大的提升。
2. 组件化机制，.vue 文件，对于开发大型的项目使其更加模块化，可读性更强，可维护性更强。
3. 入门简单，学习曲线不那么陡峭。
