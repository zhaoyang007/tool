...
有几个对象数据（包括data）就有几个 Observer 实例，dep 的数量是对象数据个数（包括 data） + data  内所有 key 的数量，几个组件就有几个 Watcher。
...
* vm.\_update()：执行更新
  ...
  * vm.\__patch__()：传入新老 vnode，做对比找出最小变化，执行 DOM 操作，返回 vnode。
    ...
    * 组件的 old vnode 树不存在就增：从上到下，创建挂载创建挂载，最后把整棵树创建完毕后一起放到页面中去，然后删除之前的宿主。虚拟 DOM 创建完成了，就要开始执行 patch 了，patch 里发现以前没有现在有，所以要执行一次创建元素，要把 vnode 批量创建成 DOM 元素，在创建 DOM 树的过程中就要做组件实例的创建和挂载。
      ...
      * createElm：把 vnode 变成真实 DOM，根组件执行更新函数时，会递归创建子元素和子组件，首次执行 _update() 时，patch() 会通过 createElm() 创建根元素。
        * createComponent()：如果要创建的是组件，走这个流程
          ...
          * insert()：该组件对应的 DOM 树插入父组件的 DOM 树上的操作
    ...
    * 都存在就执行 diff 执行更新。
      ...
      * patchVnode：
        ...
        深度优先：有孩子先比孩子调用 updateChildren，updateChildren 中还会调用 patchVnode，一直向下递归，将每个 vnode 节点都 patch 一遍。

        每个节点比较的和更新的就是三件事：属性更新，文本更新，子节点更新：

        * isPatchable(vnode)：节点属性更新。

        * 都无子节点：只是文本的替换。

        * 老无子节点，新有子节点：先清空老文本内容，然后为其新增子节点。 

        * 新无子节点，老有子节点：移除该节点的所有子节点。

        * 新老均有子节点：对子节点进行 Diff 操作，调用 updateChildren。

          * updateChildren：同级比较

            设置双指针，首尾都没有找到相同的节点还是要做双循环。
...
* 优化：optimize(ast, options)
  * 在 AST 中标记静态子树，静态子树是界面中永远不变的节点，如两层嵌套以上的纯文本节点。
    - 每次重新渲染，不需要为静态子树创建新节点。
    - 虚拟 DOM 中 patch 时，可以跳过静态子树。
* 生成：code = generate(ast, options)
  * 把 AST 转换成代码字符串，使用 new Function(code) 就可以得到一个真正的 render 函数。
