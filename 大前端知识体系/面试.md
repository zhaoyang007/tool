...
采用 transform: translateZ(0) 采用 transform: translate3d(0, 0, 0) 使用 CSS 的 will-change属性。 will-change 可以设置为 opacity、transform、top、left、bottom、right。
...
##### 实现 Promise.prototype.finally（8）2021.12.06
...
与 preload 类似，区别是浏览器会在空闲的时候，下载 main.js，当有页面使用的时候，直接从缓存中读取。如果在 prefetch 还没下载完的时候，浏览器发现 script 标签也引用了同样的资源，浏览器会再次发起请求，这样会造成加载了两次，所以不要在当前页面马上就要用的资源上用 prefetch，要用 preload。
...
侦听到数据变化，不会把对应的 Watcher 直接更新，而是开启一个异步队列，存放所有需要变化的组件 Watcher，在异步队列里，一次性的执行所有 Watcher 的更新函数，更新函数把所有的 DOM 更新全部做完之后，浏览器统一的刷新一次，就达到了我们期待的高效方式。

一个组件的值同时可能会修改多次， Watcher 被多次触发，但只会被推入到队列中一次，避免不必要的计算和 DOM 操作。

$nextTick 就是将 cb 放到这个异步队列中，这样就可以在数据变化后获取到变化后的 dom。因为 dom 操作都是在这个一步队列里做的。
...
* 处理 render > template > el 选项。
  ...
  选项中如果有 render 直接调用 mount 执行挂载；如果有 template 或 el，将它们进行一定处理最后变成 template，然后将这个 template 执行模版解析和编译，最终得到 render 函数并将其放到选项中去。
...
* 初始化全局 API：Vue.util, Vue.set, Vue.delete, Vue.nextTick, Vue.use, Vue.mixin, Vue.extend
...
* _init：
  ...
  * initRender(vm)：声明了 \_vnode, $slots, $scopedSlots, $createElement就是那个h, $nextTick，对$attrs, $listeners 做了响应化处理。
...
有几个对象数据（包括data）就有几个 Observer 实例，dep 的数量是对象数据个数包括 data 本身 + data  内所有 key 的数量，几个组件就有几个 Watcher。
...
* $mount
  * 执行 mountComponent
    ...
    * 声明更新函数 updateComponent
      * 执行 vm._render
      * 执行 vm._update，传入 vm.\_render 执行结果。
        * 判断 prevVnode 是否存在，不存在就是初始化渲染，存在就是更新过程。
        * 执行 vm.\__patch__
    * 创建组件 Watcher，传入 updateComponent
      * 执行 updateComponent
    * callHook(vm, 'mounted')

* $mount()，执行挂载，只做了 mountComponent 这一个事。
  ...
  * mountComponent()，执行挂载转换
    ...
    * 构建组件实例的时候，创建了一个和组件实例相关的 Watcher，传入更新函数 updateComponent，初始化过程 Watcher 会执行一次更新函数，以后有更新，Watcher 会让更新函数再次执行。

总结：

虚拟dom,结合diff算法能够尽量的减少dom操作的数量，从而提升性能。
...
- 解析：将 js 代码解析成抽象语法树（AST）。词法分析和语法分析。
...
```js
// 1 技巧 hash，后存 map，天然避免当前map的数重复计算
