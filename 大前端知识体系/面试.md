...
* 首屏时间：输入网址回车后的时间到 DCL 的时间。
* L（Onload Event）：页面全部元素渲染完成事件。
...
* TBT（Total Blocking Time）：阻塞总时间，记录在 FCP 到 TTI 之间所有长任务的阻塞时间总和。每个长任务的阻塞时间就等于它所执行的总时间减去 50ms。执行时间大于 50ms就是长任务，否则是短任务。
