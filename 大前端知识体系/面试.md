...
* $mount
  * 执行 mountComponent
    ...
    * 声明更新函数 updateComponent
      * 执行 vm._render
      * 执行 vm._update，传入 vm.\_render 返回的 vnode。
        * 执行 vm.\__patch__：初始化和更新。
...
* vm.\_update
  ...
  * vm.\__patch__：实际调用的是 patch。传入新老 vnode，做对比找出最小变化，执行 DOM 操作。

    * 组件的 new vnode 树不存在就删除。

    * 组件的 old vnode 树不存在就新增：从上到下，创建挂载创建挂载，最后把整棵树创建完毕后一起放到页面中去，然后删除之前的宿主。虚拟 DOM 创建完成了，就要开始执行 patch 了，patch 里发现以前没有现在有，所以要执行一次创建元素，要把 vnode 批量创建成 DOM 元素，在创建 DOM 树的过程中就要做组件实例的创建和挂载。
...
* 生成：code = generate(ast, options)
  * 把 AST 转换成代码字符串，传入 new Function(code) 中就可以得到一个真正的 render 函数。

总结：

虚拟 dom 结合 diff 算法能够尽量的减少 dom 操作的数量，从而提升性能。
